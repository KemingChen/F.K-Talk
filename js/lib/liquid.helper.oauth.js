...
(/** @lends liquid.helper */function(helper) {
	...
	helper.oauth = {
		...
		authorize: function(callBack) 
		{
			...
			inAppBrowser = window.open(authUri, '_blank', 'location=no');
		 ...
		 */
		onAuthUrlChange: function(event) {
			var $this = helper.oauth;
			console.log('onAuthUrlChange' + event.url);
			
			if (event.url.indexOf("code=") != -1) {
				$this.requestStatus = $this.status.SUCCESS;
				
				/* Store the authCode temporarily */
				$this.authCode = $this.getParameterByName("code", event.url);
		    ...
		    else if (event.url.indexOf("error=") != -1) 
		    {
		    	...
		    	$this.errorMessage = $this.getParameterByName("error", event.url);
